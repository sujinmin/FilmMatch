	...
	// 	let signup__bnt = $(".signup__bnt").hide;
	// 	// 페이지가 로드될 때 숨겨진 요소들 설정
	// 	$("#repeat__password, #mem_name, #mem_email, #mem_zipcode, .button, #mem_addr","#signup__bnt").hide();
	...
	$(document).ready(function(){
		...
		let signin = $(".links").find("li").find("#signin");
		let first_input = $("form").find(".first-input");
		let hidden_input = $("form").find(".input__block").find(".show");
		let signin_btn = $("form").find(".signin__btn");
		let insert = $(".input__block").slice(3); // 네 번째 요소부터 끝까지 모든 요소
		let signup__bnt = $(".signup__bnt");
		let signin__bnt = $(".signin__bnt");
	
		// 페이지 로드 시 숨겨진 요소 설정
		$("#repeat__password, #mem_name, #mem_email, #mem_zipcode, .button, #mem_addr").hide();
		signup__bnt.hide(); // 초기에는 회원가입 버튼 숨김
	
		// "Sign Up" 버튼 클릭 시 동작 설정
		signup.on("click", function(e) {
			e.preventDefault();
			$("h1").text("SIGN UP");
			$(".links li a").css("opacity", ".6");
			...
			hidden_input.css({ "opacity": "1", "display": "block" });
			// signin_btn.text("Sign up");
			// signup__btn.show(); // 회원가입 버튼 보이기
			insert.show();
			signup__bnt.show(); // 회원가입 버튼 숨기기
			signin__bnt.show(); // 로그인 버튼 보이기
		});
	
		// "log In" 버튼 클릭 시 동작 설정
		signin.on("click", function(e) {
			e.preventDefault();
			$("h1").text("SIGN IN");
			$(".links li a").css("opacity", ".6");
